# this file is for smeantic search
import csv
import nltk
...
model_dir ='C:/Users/leonv/Documents/development/Master/Information_retrieval/GoogleNews-vectors-negative300.bin/GoogleNews-vectors-negative300.bin'
w2v_model = gensim.models.KeyedVectors.load_word2vec_format(model_dir, binary=True)  
...
def querysearch(INDEX, TITLES, query, food=False, transport=False, culture=False, continent=None):
    ...
    for word in set(tokens2): # not stemmed tokens for w2v simularity
        if word in w2v_model.wv.vocab:
            top_simular = w2v_model.most_similar(positive=[word], topn=3) # get top N simular words
            for sim_word in top_simular:
                stemmed = ps.stem(sim_word[0]) # now stem them
    ...
    for doc in ranking[:10]:
        print("{} ({})".format(doc['title'], doc['url']))
...
# print(querysearch(INDEX,TITLES, "Hello World"))
